...
@Component({
  ...
  template: `
    <div class="mb-6 flex items-center justify-between">
      <h2 class="text-lg font-semibold text-gray-800 dark:text-white/90">Subjects</h2>
    ...
    <div class="mb-4 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]" *ngIf="showForm">
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800" [(ngModel)]="form.code" />
        </div>
      </div>
        ...
        <table class="w-full table-auto">
          <thead>
            <tr class="border-b border-gray-100 dark:border-gray-800">
              ...
              <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">Code</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            <tr *ngFor="let s of items" class="hover:bg-gray-50 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-5 py-3 text-sm text-gray-700 dark:text-gray-300">{{s.code}}</td>
