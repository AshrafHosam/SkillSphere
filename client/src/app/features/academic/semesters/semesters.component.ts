...
@Component({
  ...
  template: `
    <div class="page-header"><h1>Semesters</h1>
    ...
    <div class="card"><table class="data-table"><thead><tr><th>Name</th><th>Start</th><th>End</th><th>Current</th><th>Actions</th></tr></thead>
      <tbody><tr *ngFor="let s of items"><td>{{s.name}}</td><td>{{s.startDate | localDate:'mediumDate'}}</td><td>{{s.endDate | localDate:'mediumDate'}}</td>
        ...
        <td><button class="btn-sm btn-danger" (click)="remove(s.id)">Delete</button></td></tr></tbody></table></div>
  `,
  styles: [`.page-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem}.page-header h1{margin:0}.btn-primary{padding:.5rem 1rem;background:#0f172a;color:#fff;border:none;border-radius:6px;cursor:pointer}.btn-sm{padding:.25rem .75rem;border:none;border-radius:4px;cursor:pointer;font-size:.8rem}.btn-danger{background:#ef4444;color:#fff}.card{background:#fff;padding:1.5rem;border-radius:8px;box-shadow:0 1px 3px rgba(0,0,0,.1);margin-bottom:1rem}.form-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:1rem}.form-group{margin-bottom:1rem}.form-group label{display:block;margin-bottom:.25rem;font-weight:600;font-size:.875rem;color:#334155}.form-group input{width:100%;padding:.5rem;border:1px solid #e2e8f0;border-radius:4px;box-sizing:border-box}.data-table{width:100%;border-collapse:collapse}.data-table th,.data-table td{padding:.75rem;text-align:left;border-bottom:1px solid #e2e8f0}.data-table th{font-weight:600;color:#64748b;font-size:.8rem;text-transform:uppercase}.badge-active{background:#dcfce7;color:#166534;padding:.2rem .5rem;border-radius:12px;font-size:.75rem}`]
})
