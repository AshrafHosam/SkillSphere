...
@Component({
  selector: 'app-header',
  standalone: true,
  imports: [CommonModule, RouterModule, ThemeToggleButtonComponent, DropdownComponent],
  template: `
      ...
      <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div class="flex items-center justify-between w-full gap-2 px-3 py-3 border-b border-gray-200 dark:border-gray-800 sm:gap-4 xl:justify-normal xl:border-b-0 xl:px-0 lg:py-4">
          ...
          <button
            ...
            [ngClass]="{ 'bg-gray-100 dark:bg-white/[0.03]': isMobileOpen$ | async }"
          ...
          <!-- Mobile Logo -->
          <a routerLink="/dashboard" class="xl:hidden">
            <span class="text-xl font-bold text-brand-500">SkillSphere</span>
          </a>
          <div class="xl:hidden w-10"></div>
        </div>
        <div class="flex items-center justify-end w-full gap-4 px-5 py-4 xl:flex xl:px-0">
          <div class="flex items-center gap-2">
            <app-theme-toggle-button />
          </div>
          <!-- User Dropdown -->
          <div class="relative">
            <button (click)="toggleUserDropdown()" class="flex items-center text-gray-700 dropdown-toggle dark:text-gray-400">
              <span class="mr-3 flex items-center justify-center overflow-hidden rounded-full h-11 w-11 bg-brand-500 text-white font-semibold text-sm">
                {{ getInitials() }}
              </span>
              <span class="block mr-1 font-medium text-theme-sm">{{ auth.fullName }}</span>
              <svg [ngClass]="{ 'rotate-180': isUserDropdownOpen }" width="18" height="20" viewBox="0 0 18 20" fill="none" class="transition-transform duration-200">
            ...
            <app-dropdown [isOpen]="isUserDropdownOpen" (close)="closeUserDropdown()"
              className="absolute right-0 mt-[17px] flex w-[260px] flex-col rounded-2xl border border-gray-200 bg-white p-3 shadow-theme-lg dark:border-gray-800 dark:bg-gray-dark">
              <div>
                <span class="block font-medium text-gray-700 text-theme-sm dark:text-gray-400">{{ auth.fullName }}</span>
                <span class="mt-0.5 block text-theme-xs text-gray-500 dark:text-gray-400">{{ auth.currentUser?.user?.email }}</span>
                <span class="mt-0.5 block text-theme-xs text-brand-500">{{ auth.userRole }}</span>
              </div>
              <hr class="my-3 border-gray-200 dark:border-gray-800" />
              <button (click)="signOut()"
                class="flex items-center gap-3 px-3 py-2 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300 w-full">
                <svg class="fill-gray-500 group-hover:fill-gray-700 dark:group-hover:fill-gray-300" width="24" height="24" viewBox="0 0 24 24" fill="none"><path fill-rule="evenodd" clip-rule="evenodd" d="M15.1007 19.247C14.6865 19.247 14.3507 18.9112 14.3507 18.497L14.3507 14.245H12.8507V18.497C12.8507 19.7396 13.8581 20.747 15.1007 20.747H18.5007C19.7434 20.747 20.7507 19.7396 20.7507 18.497L20.7507 5.49609C20.7507 4.25345 19.7433 3.24609 18.5007 3.24609H15.1007C13.8581 3.24609 12.8507 4.25345 12.8507 5.49609V9.74501L14.3507 9.74501V5.49609C14.3507 5.08188 14.6865 4.74609 15.1007 4.74609L18.5007 4.74609C18.9149 4.74609 19.2507 5.08188 19.2507 5.49609L19.2507 18.497C19.2507 18.9112 18.9149 19.247 18.5007 19.247H15.1007ZM3.25073 11.9984C3.25073 12.2144 3.34204 12.4091 3.48817 12.546L8.09483 17.1556C8.38763 17.4485 8.86251 17.4487 9.15549 17.1559C9.44848 16.8631 9.44863 16.3882 9.15583 16.0952L5.81116 12.7484L16.0007 12.7484C16.4149 12.7484 16.7507 12.4127 16.7507 11.9984C16.7507 11.5842 16.4149 11.2484 16.0007 11.2484L5.81528 11.2484L9.15585 7.90554C9.44864 7.61255 9.44847 7.13767 9.15547 6.84488C8.86248 6.55209 8.3876 6.55226 8.09481 6.84525L3.52309 11.4202C3.35673 11.5577 3.25073 11.7657 3.25073 11.9984Z" fill=""/></svg>
                Sign Out
              </button>
            </app-dropdown>
          </div>
...
export class AppHeaderComponent {
  ...
  closeUserDropdown() { this.isUserDropdownOpen = false; }
  signOut() { this.isUserDropdownOpen = false; this.auth.logout(); }
}
